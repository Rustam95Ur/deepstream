...
Canonical store is ``NodeSettings.trigger_thresholds``. Flat fields on
``NodeSettings`` (min_tracks, cooldown_s, …) are API/settings.json mirrors
kept in sync by ``merge_trigger_thresholds`` + ``sync_flat_from_profiles``.
"""
...
# Hardcoded defaults — source of truth when both flat and profiles are empty.
DEFAULT_THRESHOLD_PROFILES: dict[str, dict[str, float | int]] = {
    "presence": {
        "presence_min_people": 1,
        "presence_sustain_s": 2.0,
        "cooldown_s": 30.0,
    },
    "convergence": {
        "min_tracks": 2,
        "converge_dist_bh": 1.5,
        "speed_thresh_bh": 2.0,
        "sustain_s": 0.4,
        "cooldown_s": 30.0,
    },
...
def flat_threshold_defaults(settings: Any) -> dict[str, dict[str, Any]]:
    """Legacy flat NodeSettings fields overlaid on hardcoded defaults."""
    base = {k: dict(v) for k, v in DEFAULT_THRESHOLD_PROFILES.items()}
...
def merge_trigger_thresholds(
    ...
    """defaults ← flat mirrors ← ``trigger_thresholds`` (wins)."""
...
def sync_flat_from_profiles(settings: Any) -> None:
    """Mirror profiles back onto legacy flat fields (API / old UI)."""
